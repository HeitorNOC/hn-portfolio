/* ═══════════════════════════════════════════════════════════
   HN Services — FlowWave (interlúdio "Interação")
   ─────────────────────────────────────────────────────────
   PORTE FIEL de Desktop/flow-wave/index.html. Shaders, composers,
   bloom seletivo por layers, chamas do FinalPass e motes são os
   do original, intocados.

   Adaptações (e só estas):
   • imports do unpkg → js/libs/three/ vendorizado  (offline-first, §5)
   • #scene → #flow-canvas, dimensionado pela seção
   • fly-path de 620vh → câmera fixa (FIXED_SCROLL) em 1 tela
   • devicePixelRatio capado em 1.5  (§3C)
   • loop pausado por IntersectionObserver + visibilitychange (§3A)
   ═══════════════════════════════════════════════════════════ */

/* Specifiers "three" e "three/addons/" são resolvidos pelo <script type="importmap">
   do index.html, que aponta para js/libs/three/ (vendorizado, offline-first).
   Precisa ser specifier nu: os próprios addons importam de 'three' — usar
   caminho relativo aqui carregaria uma SEGUNDA cópia do Three em memória. */
...
/* Flow dirigido pelo scroll: quanto a onda avança ao percorrer a seção
   inteira, e a deriva mínima que a mantém viva com o scroll parado. */
const STREAM_PER_SCROLL = 240
const IDLE_DRIFT        = 1.2

/* A viagem termina em FLY_END (a onda assenta na horizontal) e o slam entra
   logo em seguida. O que sobra até 1.0 é tempo de tela para APRECIAR o final:
   onda horizontal correndo + "HN Services" aceso. ~40% da seção. */
const FLY_END = 0.56
const SLAM_AT = 0.60
...
// ---------------------------------------------------------------------------
// Shared GLSL: 3D Simplex noise (injected into the points vertex shader)
// ---------------------------------------------------------------------------
const SNOISE = `
...
const camera = new THREE.PerspectiveCamera(45, sizeOf().w / sizeOf().h, 0.1, 400)
...
// ---------------------------------------------------------------------------
// Points geometry + material (reshaped into a flat noise sheet in the shader)
// ---------------------------------------------------------------------------
const pointsVertex = `
...
;(function () {
  ...
  atmoMat = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uRes:   { value: new THREE.Vector2(sizeOf().w * Math.min(window.devicePixelRatio, 2), sizeOf().h * Math.min(window.devicePixelRatio, 2)) }
    },
    vertexShader: `
...
torusComposer.addPass(new UnrealBloomPass(new THREE.Vector2(sizeOf().w, sizeOf().h), 0.22, 0.2, 0))
torusComposer.addPass(new ShaderPass(CopyShader))

const bloomComposer = new EffectComposer(renderer)
bloomComposer.renderToScreen = false
bloomComposer.addPass(renderPass)
bloomComposer.addPass(new UnrealBloomPass(new THREE.Vector2(sizeOf().w, sizeOf().h), 0.4, 0.55, 0))
...
/* O original mapeia o fly-path ao scroll da PÁGINA inteira (620vh).
   Aqui o mesmo percurso é mapeado ao scroll da PRÓPRIA SEÇÃO (~220vh):
   a câmera faz a viagem completa — de longe até atravessar a onda —
   sem alongar o site. É daqui que vem a fluidez. */
const slamEl = document.getElementById('flow-slam')
const hintEl = document.querySelector('.flow__hint')

function recomputeScroll() {
  if (!section) return
  const total = section.offsetHeight - window.innerHeight
  const passed = -section.getBoundingClientRect().top
  scrollTarget = total > 0 ? clamp(passed / total, 0, 1) : 0

  // Slam do "HN Services" no fim da onda (a classe dispara a animação CSS)
  if (slamEl) slamEl.classList.toggle('is-slam', scrollTarget >= SLAM_AT)
  if (hintEl) hintEl.style.opacity = scrollTarget >= SLAM_AT ? '0' : '1'
}
window.addEventListener('scroll', recomputeScroll, { passive: true })
recomputeScroll()
scrollSmooth = scrollCurrent = scrollTarget
...
// ---------------------------------------------------------------------------
// Pointer projection onto z = 0 plane in world space
// ---------------------------------------------------------------------------
...
const sceneObj = {
  ...
  render(scroll, m) {
    ...
    // O FLOW é dirigido pelo SCROLL (no original ele corria só com o tempo):
    // rolar empurra as colinas em direção à câmera. Uma deriva mínima de tempo
    // fica só para a onda não morrer quando o visitante para de rolar.
    this.stream = scroll * STREAM_PER_SCROLL + t * IDLE_DRIFT
    pointsUniforms.uStream.value = this.stream
    pointsUniforms.uWaveHeight.value = waveHeight * (1 + scroll * scrollRise)

    // Camera fly-path: smoothstep até FLY_END (a onda assenta na horizontal lá).
    const ea = Math.min(scroll / FLY_END, 1.0)
...
/* Só renderiza enquanto a seção está em cena. */
function start() { if (running) return; running = true; canvas.style.visibility = 'visible'; animate() }
function stop()  { running = false; cancelAnimationFrame(rafId); canvas.style.visibility = 'hidden' }
